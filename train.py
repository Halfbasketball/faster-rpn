...
image_height, image_width = 900, 900
...
def create_image_label_path_generator(synthetic_dataset_path):
    labelfile='/data/exp_900/ImageSets/train.txt'
    labelpath='/data/exp_900/Annotations'
    f= open(labelfile,'r')
    labeldata=f.readlines()
    f.close()
    image_label_paths = [(os.path.join(synthetic_dataset_path,labeldata[idx], ".png" ),
                          os.path.join(labelpath, labeldata[idx], ".xml")) for idx in range(len(labeldata))]
    while True:
...
def DataGenerator(synthetic_dataset_path, batch_size):
    ...
    while True:
        ...
        target_scores = np.zeros(shape=[batch_size, wnum, hnum, 9, 2], dtype=np.float)
        target_bboxes = np.zeros(shape=[batch_size, wnum, hnum, 9, 4], dtype=np.float)
        target_masks  = np.zeros(shape=[batch_size, wnum, hnum, 9], dtype=np.int)
...
def compute_loss(target_scores, target_bboxes, target_masks, pred_scores, pred_bboxes):
    """
    target_scores shape: [1, wnum, hnum, 9, 2],  pred_scores shape: [1, wnum, hnum, 9, 2]
    target_bboxes shape: [1, wnum, hnum, 9, 4],  pred_bboxes shape: [1, wnum, hnum, 9, 4]
    target_masks  shape: [1, wnum, hnum, 9]
    """
...
batch_size = 25
lambda_scale = 1.
synthetic_dataset_path="/data/exp_900/Images"
